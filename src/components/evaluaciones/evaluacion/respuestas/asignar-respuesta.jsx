    ...
    useEffect(() => {
      ...
      useEffect(() => {
        ...
        const getRespuestas = async () => {
            ...
            const { data, error } = await supabase
            .from("respuestas")
            .select("descripcion, id_respuesta")
            .eq("id_pregunta", id_pregunta)      
  ...
  const handleDelete = async (id_respuesta) => {
    const { data, error } = await supabase
      ...
      const handleEdit = async (id_respuesta) => {
        const { data, error } = await supabase
          .from("respuestas")
          .select("tipo_respuesta, descripcion, correcta, id_pregunta, id_respuesta")
    ...
    return (
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-8 mt-8 mb-8 mx-auto max-w-7xl">
         <div>
        <h1 className="text-center text-xl font-bold">Crear respuestas para la pregunta</h1>
      </div>
      <div>
      ...
      <div className="w-1/2 p-4">
        ...
        <h2 className="text-center text-xl font-bold">Respuestas asignadas a la pregunta</h2>
        <div>
      <span className="font-bold mb-4">Pregunta: </span>
      {nombrePregunta}
      </div>
        <div className="space-y-4 text-center">
          ...
          {respuestas.map((respuesta, index) => (
            <TableRow key={index}>
              ...
              <TableCell>
                <Button onClick={() => handleEdit(respuesta.id_respuesta)} className="w-1/2" type="submit">
                  Edit
                  </Button>
                  <Button onClick={() => handleDelete(respuesta.id_respuesta)} className="w-1/2" type="submit">
                  Delete
                </Button>
              </TableCell>
            </TableRow>
